...
#define JSTP_RX_BUFFER_SIZE 64
// size of outpuy character buffer
#define JSTP_TX_BUFFER_SIZE 64
// size of JSON tokens array (one token size - sizof(int)*4+1)
#define JSTP_JS_TOKENS_SIZE 30
...
// check buffer and execute tasks (call as many as possible)
void jstp_tick();

// compare two null-terminal string
int jstp_str_compare(char *x, const char *y);

// get char from tx buffer. return bool - is tx buffer contain data to read
